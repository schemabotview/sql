...
//
// FOCUS: this node read `__focus` but never painted it, so `Section.focus` pointing at a tile was a
// silent no-op — every other renderer (SceneNode, TableNode, CodeNode) glows. Found when the SQL
// capstone used focus to light one step of a tile strip and nothing happened. A tile has no chrome
// to thicken, so focus gives it its own: a tinted rounded plate plus the same accent ring the cards
// use, which reads as "this one" without turning the tile into a card.
