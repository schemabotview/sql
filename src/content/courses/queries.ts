...
export const queries: Course = {
  ...
  sections: [
    ...
    {
      // ── §2 FROM — the first stage: choose the source rows. `focus: []` frames the WHOLE scene
      //    (empty → WHOLE_PADDING, no zoom-out) with an empty lit-set (no dim) — identical framing
      //    to §1. The slide names the stage in play. ──
      id: 'from',
    ...
    },
    {
      ...
      //    the reveal reset). Re-solidify the WHOLE pipeline to restore the solid tour, then frame
      //    the run band — no dim, every stage stays lit. ──
      id: 'where',
      heading: 'WHERE: filtering rows',
      scene: 'query-pipeline',
      // `focus: []` → whole scene framed (WHOLE_PADDING, no zoom-out) + empty lit-set (no dim),
      // like §1. (This re-entry delta also re-solidifies the whole pipeline after the detour.)
      focus: [],
      slide: {
    ...
    {
      // ── §5 GROUP BY + HAVING — consecutive same-scene section: solidify the two grouping stages;
      //    frame the whole scene (`focus: []`), no dim (every stage stays lit). ──
      id: 'group-having',
      heading: 'GROUP BY & HAVING: aggregation',
      scene: 'query-pipeline',
      // `focus: []` → whole scene framed (WHOLE_PADDING, no zoom-out) + empty lit-set (no dim), like §1.
      focus: [],
      slide: {
    ...
    },
    {
      ...
      //    then frame the run band — no dim, every stage stays lit. ──
      id: 'select-distinct',
    ...
    },
    {
      ...
      //    the two arranging stages; frame the whole scene (`focus: []`), no dim (every stage stays lit). ──
      id: 'order-limit',
